...
const char * const BITCOIN_CONF_FILENAME = "pixelcoin.conf";
const char * const BITCOIN_PID_FILENAME = "pixelcoin.pid";
...
#else
    const char* pszModule = "pixelcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Pixelcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Pixelcoin";
#else
    // Unix
    return pathRet / ".pixelcoin";
#endif
#endif
}
