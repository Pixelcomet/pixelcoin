Pixelcoin Core version *0.15.1* is now available from:

  <https://download.pixelcoin.org/pixelcoin-0.15.1/>
...
A number of changes to the way Pixelcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 pixels per kB (0.00001 PXC). This is to prepare for dropping the minimum transaction fee to 0.1 pixels per kB (0.0001 PXC) in the next release.
