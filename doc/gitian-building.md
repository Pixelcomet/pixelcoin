...
*Setup instructions for a Gitian build of Pixelcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Pixelcoin
...
- [Building Pixelcoin Core](#building-pixelcoin-core)
...
Clone the git repositories for pixelcoin and Gitian.
...
in the pixelcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Pixelcoin Core
----------------

To build Pixelcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the pixelcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/pixelcoin/.git/
...
For example:
```bash
URL=https://github.com/thrasher-/pixelcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit pixelcoin=${COMMIT} --url pixelcoin=${URL} ../pixelcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit pixelcoin=${COMMIT} --url pixelcoin=${URL} ../pixelcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit pixelcoin=${COMMIT} --url pixelcoin=${URL} ../pixelcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the pixelcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../pixelcoin/contrib/gitian-descriptors/*|sort|uniq )
...
./bin/gbuild --url pixelcoin=${BTCPATH},signature=${SIGPATH} ../pixelcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/pixelcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/pixelcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/pixelcoin-osx-build.assert
```
...
[pixelcoin-project/gitian.sigs.ltc](https://github.com/pixelcoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
